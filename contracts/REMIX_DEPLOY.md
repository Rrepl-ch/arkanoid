...
2. Под кнопкой **Deploy** появится поле конструктора **\_treasury** (address). Вставь сюда **адрес кошелька, на который должны приходить деньги** с продажи платных шаров (Emerald, Ruby, Gold). Например: `0x1234...` (твой адрес или мультисиг).
3. Нажми **Deploy**.
4. Подтверди транзакцию в MetaMask.
5. После успешного деплоя скопируй **адрес контракта** (под надписью контракта, иконка копирования).

Сохрани адрес контракта — понадобится для `.env` как `VITE_ARKANOID_BALLS_ADDRESS`.
